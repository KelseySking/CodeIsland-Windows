...
稳定边界是 `docs/runtime-display-contract.md` 中描述的本地 CodeOrbit API。展示端应该依赖 REST DTO、WebSocket 事件和操作端点，不应该依赖 WPF ViewModel、Hub 具体类、Named Pipe hook 处理、transcript 文件或 source-specific 响应构造器。
...
WPF HUD 在 managed 模式下会启动内嵌 host（优先 `codeorbit-host.exe`，兼容历史 `CodeOrbit.RuntimeHost.exe` 等），并把本地 API token 写入：
...
独立 CodeOrbit 开发时，可以显式指定 token（示例路径随 CodeOrbit 发行物变化）：

```powershell
# 以当前发行物中的 host 可执行文件为例
./codeorbit-host.exe --token dev-token --host 127.0.0.1 --port 32145
```
...
`GET /api/health` 不需要认证。其它端点都需要 CodeOrbit API token。
...
1. 用 `GET /api/health` 检查 CodeOrbit 是否存活。
...
当 `resolved` 为 `false` 时，保留这个 pending action，并重新获取 `/api/pending`；CodeOrbit 已经推进到下一个问题步骤。当 `resolved` 为 `true` 时，下一次 pending 快照会移除这个 action。
...
* 如果展示端需要 CodeOrbit 当前没有暴露的数据，应扩展公共 contract，而不是读取 WPF、Hub、Core、Bridge、transcript 或 settings 内部实现。
...
示例内部定义了匹配公开 JSON contract 的最小本地 DTO。正式 SDK 包在 CodeOrbit contract 稳定到可发布之前仍不属于当前范围。
